...
📋 **[CHANGELOG.md](CHANGELOG.md)** has the full version history (currently v3.12). 📖 **[DOCUMENTATION.md](DOCUMENTATION.md)** has a complete reference of every command and system in detail. This README stays focused on setup and a feature overview.
...
- [Personas](#personas)
- [Persona Webhook Identities](#persona-webhook-identities)
- [Breaking Character: the `noper` prefix](#breaking-character-the-noper-prefix)
...
- [Custom App Emojis](#custom-app-emojis)
...
├── railway.toml
├── .env.example
├── README.md This file — setup + feature overview
├── CHANGELOG.md Full version history
├── DOCUMENTATION.md Complete reference: every command and system in detail
└── CLAUDE.md AI-assistant-oriented reference (Claude Code reads this automatically)
```
...
`/affection mood` is the one built for casually checking in without spoiling the fun with exact numbers — rather than a bare score, it posts publicly as a generated card: the persona's art centered over a background, with a short line like "Frieren seems to have a decent impression of you" on a frosted glass panel underneath. The phrasing at each level can be overridden per persona (see `moodPhrases` on the `Persona` type) for characters where the generic "smitten"/"resents you" framing doesn't fit — every persona in the roster has at least a custom top and bottom level written for their own voice, with a handful going further where their personality earns a fuller arc (see `DOCUMENTATION.md` for the specifics of who and why). Backgrounds are per-persona — drop an image in `backgrounds/` named after that persona's avatarKey, same convention as `avatars/` (see `backgrounds/README.md`) — and personas without one yet just fall back to a generated gradient. `/affection view` is the precise, numeric version for admins who want to actually audit or debug it, and stays private and Admin+ like before.
